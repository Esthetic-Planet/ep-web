 ...
 if(isset($_POST['submit']) && ($_REQUEST['submit'] == "Submit"))
{
	...
	if($intResult)
	{
		...
		{
		if($_FILES['face_fwd']['size'] >2000000 || $_FILES['face_fwd']['error'])
	{
	header("Location: hair_form.php?mseg=fwd&hid=".$_REQUEST['hid']."&qid=".$_REQUEST['qid']."&clid=".$_REQUEST['clid']);
	die();
	}
		...
		{
		if($_FILES['top_skull']['size'] >2000000 || $_FILES['top_skull']['error'])
	{
	header("Location: hair_form.php?mseg=skull&hid=".$_REQUEST['hid']."&qid=".$_REQUEST['qid']."&clid=".$_REQUEST['clid']);
	die();
	}
				$upload_dir= _UPLOAD_FILE_PATH."mail_attachment/";
		...
		{
		if($_FILES['face_side']['size'] >2000000 || $_FILES['face_side']['error'])
	{
	header("Location: hair_form.php?mseg=side&hid=".$_REQUEST['hid']."&qid=".$_REQUEST['qid']."&clid=".$_REQUEST['clid']);
	die();
	}
				$upload_dir= _UPLOAD_FILE_PATH."mail_attachment/";
		...
		{
		if($_FILES['donor_area']['size'] >2000000 || $_FILES['donor_area']['error'])
	{
	header("Location: hair_form.php?mseg=donor&hid=".$_REQUEST['hid']."&qid=".$_REQUEST['qid']."&clid=".$_REQUEST['clid']);
	die();
	}
				$upload_dir= _UPLOAD_FILE_PATH."mail_attachment/";
	...
	header("Location: hairfrm_detail.php?msg=thk");
	die();
	}
...
} else  if(isset($_POST['submit']) && (($_REQUEST['submit'] == "Update")||($_REQUEST['submit'] == "envoyer")))
{
...
else {  if($attachmants != "" && $Data_list["face_fwd"]!="")	  $attachmants .= "|";
				$attachmants .= $Data_list["face_fwd"] ; }
	...
	}else {  if($attachmants != "" && $Data_list["top_skull"]!="")	  $attachmants .= "|";
				$attachmants .= $Data_list["top_skull"] ; }
	if($_FILES['face_side']['name']!="")
	{
	...
	}else {  if($attachmants != "" && $Data_list["face_side"]!="")	  $attachmants .= "|";
				$attachmants .= $Data_list["face_side"] ; }
	if($_FILES['donor_area']['name']!="")
	{
	if($_FILES['donor_area']['size'] >2000000 || $_FILES['donor_area']['error'])
	{
	...
	} else{  if($attachmants != "" && $Data_list["donor_area"]!="" )	  $attachmants .= "|";
				$attachmants .= $Data_list["donor_area"] ; }
...
if($_REQUEST['mseg']!="")
{
	if($_REQUEST['mseg']=="fwd") $mesg="File Size Error In "."Veuillez entrer une des images vers l'avant";
	if($_REQUEST['mseg']=="skull") $mesg="File Size Error In "."Entrez une image du sommet de votre cr&#65533;ne";
	if($_REQUEST['mseg']=="side") $mesg="File Size Error In "."Entrez une image sur les c&#65533;t&#65533;s";
	if($_REQUEST['mseg']=="donor") $mesg="File Size Error In "."Entrez une photo de vous zone donneuse";
}
?>
						...
						<ul>
							...
							<li class="field1_form">
								<select name="transplant_procedure" class="listmenu">
									...
									<option value="FUE(scarless)" <?php if($Data_list["transplant_procedure"]=="FUE(scarless)"){?> selected="selected" <?php } ?>>FUE (sans cicatrice)</option>
