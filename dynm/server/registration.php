...
if(isset($_POST['submit']) && $_POST['submit']=='Mettre à jour maintenant')
{
	...
	if(!empty($_REQUEST['alert_customer']))
	{
			...
			$email_subject="Updated registration details: "._WEBSITE_NAME;
			$email_body .=
				'
					<table width="70%" cellpadding="0" border="0" >
						<tr>
							<td colspan="2" align="left">Hi '.trim($_POST['cust_fname']).' '.trim($_POST['cust_lname']).',<br/><br/><b>You are a successful updated user on '._WEBSITE_NAME.' !! </b><br/><br/></td>
						</tr>
						<tr>
							<td colspan="2" align="left">You can login into our website by using the following details: </td>
						</tr>
						<tr>
	...
	if( ! $sql->checkCustExistsInsert(trim($_POST['cust_email'])) )
	{
		...
		if($record_id)
		{
			...
			$email_body .=
				...
				<table width="70%" cellpadding="0" border="0" >
				<tr>
					<td colspan="2" align="left">Hi,<br/><br/><b>New user is registered on '._WEBSITE_NAME.' !! </b><br/><br/></td>
				</tr>
				<tr>
					<td colspan="2" align="left">The details are given below: </td>
				</tr>
				<tr>
		...
		if(!empty($_REQUEST['alert_customer']))
		{
			...
			$email_body .=
				'
					<table width="70%" cellpadding="0" border="0" >
						<tr>
							<td colspan="2" align="left">Hi '.trim($_POST['cust_fname']).' '.trim($_POST['cust_lname']).',<br/><br/><b>You are a successful registered user on '._WEBSITE_NAME.' !! </b><br/><br/></td>
						</tr>
						<tr>
							<td colspan="2" align="left">You canlogin into our website by using the following details: </td>
						</tr>
						<tr>
							<td align="left"><b>User Name :</b></td>
							<td align="left">'.trim($_POST['cust_email']).'</td>
			...
			echo "<body>";
			echo '<form name="frmSu" id="frmSu" method="get" action="'._WWWROOT.'index.php">';
			echo '<input type="hidden" name="msg" id="msg" value="Registered">';
			echo '</form>';
			echo '<script type="text/javascript">document.frmSu.submit();</script>';
			echo '</body>';
			exit;
	...
	<div id="middle_area">
		...
		<div id="right_part">
			...
			<ul>
				...
				<li class="field"><input type="text" class="textbox" name="cust_fname" value="<?=$Data_list["cust_fname"]?>" id="chk_fname" title="Please enter the First Name"/>
				</li>
			</ul>
			<div class="clear"></div>
			<ul>
			...
			<ul>
				...
				<li class="field"><input type="text" class="textbox" name="cust_email" value="<?=$Data_list["cust_email"]?>" id="chkemail_email" title="Please enter a valid e-mail Address" />
				</li>
			</ul>
			<div class="clear"></div>
			<ul>
			...
			<div class="clear"></div>
			<ul>
				<li class="field_name">Fermeture éclair :</li>
  	...
  	<?php  }	?>
			...
			<ul>
				...
				<li class="field"><input type="password" class="textbox" name="cust_password" value="<?=$Data_list["cust_password"]?>" id="chk_password" title="Pleaes enter the Password" />
				</li>
			</ul>
			<div class="clear"></div>
			<ul>
				<li class="field_name">Retapez Pass :</li>
				<li class="field"><input type="password" class="textbox"  value="<?=$Data_list["cust_password"]?>" name="cust_cpassword" id="chkpass_repassword" title="Pleaes enter the Confirm Password" />
				</li>
			</ul>
			<div class="clear"></div>
			<ul>
				<li class="field_name">&nbsp;</li>
				<li class="field"><input type="checkbox" value="1" id="alert_customer" name="alert_customer">alertez moi par email de mes messages et devis</li>
			</ul>
			<div class="clear"></div>
			<ul>
				<li class="field_name">&nbsp;</li>
				...
				{
					$btnName = "Mettre à jour maintenant";
				}
				else
				{
				...
				<li class="field">
					...
					<input name="" type="reset" class="submit"  value="Claire"/> </li>
		  	</ul>
			<div class="clear"></div>
		  </div>
		  </form>
