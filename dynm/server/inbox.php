...
$cond = "WHERE A.mail_reciever = ". $_SESSION["UserInfo"]["id"]." and A.mail_sender=U.UserId and mail_RecieverDelete = 0  ";
$cond1 = "WHERE mail_reciever = ". $_SESSION["UserInfo"]["id"]." and mail_RecieverDelete = 0  ";
$orderby = " ORDER BY ".$sortOrder;
$total_Page = $sql->SqlRecords("esthp_mails",$cond1,$orderby,$offset,$records_per_page);
$count_total=$total_Page['TotalCount'];

$query = "SELECT A.*, U.* FROM esthp_mails as A,esthp_tblUsers as U  ".$cond.$orderby." limit $offset,$records_per_page" ;
...
if($_REQUEST['task']=='deleteAll' && (count($_REQUEST['chk'])>0))
{
	...
	for($i=0;$i<count($messages);$i++)
	{
		...
		{
			//if( $Data_del["mail_reciever"] == 1) 
			if( $Data_del["mail_receiver"] == $_SESSION["UserInfo"]["id"])
			{
				$delid=$_REQUEST['delid'];
				$ConArr['mail_RecieverDelete'] = 1;
				$CondArr = " WHERE mail_Id = '$id_del' ";
...
<?php include("header.php"); ?>
	...
	<div id="middle_area">
		...
		<div id="right_part">
			<div id="content_area">
				<div class="login_hea">Boite de réception</div>
				<div class="clear"></div>
								...
								<div id="clinicdiv">
									...
									<tr>
										<td valign="top" width="10%" align="center">
											<a href="javascript:void(0)" class="add_del" onClick="multiDelete(document.frmMails)">Supprimer les messages sélectionnés.</a>
										</td>
											 	...
											 	<tr height="25" <?=($i%2==0)? 'class="grey_bg"' : ""?>>
													...
													<td align="left" class="black_text">
														<a href="view-message.php?MId=<?php echo $Data[$i]['mail_Id']; ?>">
															<?php echo $Data[$i]['FirstName']." ".$Data[$i]['LastName'];?>
														</a>
													</td>
													<td align="left" class="black_text" nowrap="nowrap">
														<a href="view-message.php?MId=<?php echo $Data[$i]['mail_Id']; ?>">
															<?php echo $Data[$i]['mail_subject']; ?>
														</a>
													</td>
