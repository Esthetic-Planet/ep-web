...
if($_REQUEST['delid']!="")
{
			...
			$arrDest_del = $sql->SqlSingleRecord('mos_destSideImages',$ConArr_del);
		...
		if($count_del>0)
		{
			...
			$cond = " id = ".$delid;
			$sql->SqlDelete('mos_destSideImages',$cond);	
			unlink(_UPLOAD_FILE_PATH."/dest_img/DestSideImages/".$Data_del['image_name']);
		echo "<body>";
		echo '<form name="frmSu" id="frmSu" method="post" action="'._ADMIN_WWWROOT.'DestSideImages.php">';
		echo '<input type="hidden" name="msg" id="msg" value="deleted">';
		echo '<input type="hidden" name="destId" id="destId" value="'.$destId.'">';
...
{		
		...
		$ConArr['dest_id'] =$destId;
		$intResult = $sql->SqlInsert('mos_destSideImages',$ConArr);
		if($intResult)
		{
			if($_FILES['image_name']['name']!="")
			{
				...
				// in its *temporary* location in the server (often, it is /tmp)
					...
					//$handle->image_ratio_y        = true;
						...
						$handle->Process(_UPLOAD_FILE_PATH."/dest_img/DestSideImages/");
						...
						if ($handle->processed) {
							...
							$intResult = $sql->SqlUpdate('mos_destSideImages',$ConArr,$CondArr);
				...
				// in its *temporary* location in the server (often, it is /tmp)
					...
					//$handle->image_ratio_y        = true;
						...
						if ($handle->processed) {
							...
							$CondArr = " WHERE id = ".$_REQUEST['id'];
							$intResult = $sql->SqlUpdate('mos_destSideImages',$ConArr,$CondArr);
					 		@unlink(_UPLOAD_FILE_PATH."/dest_img/DestSideImages/".$_REQUEST['old_image_name']);
						// we delete the temporary files
        				$handle-> Clean();
						}
						else { $sqlError .='  Error: ' . $handle->error . ''; }
	...
	if(isset($_REQUEST['did']) && ($_REQUEST['mode']=='edit'))
	{
		$id = $_REQUEST['did'];
		$ConArr = " WHERE did= '$id' "; 						
		$arrBrands = $sql->SqlSingleRecord('mos_tblDestinations',$ConArr);
...
if(isset($_REQUEST['id']) && ($_REQUEST['mode']=='edit'))
	{
		$id = $_REQUEST['id'];
		$ConArr = " WHERE id= '$id' "; 						
		$arrBrands = $sql->SqlSingleRecord('mos_destSideImages',$ConArr);
		$count = $arrBrands['count'];
		$Data = $arrBrands['Data'];		 
	}
...
<form name="frmAddDest" action="" method="post" onsubmit="return SubmitForm(this);" enctype="multipart/form-data">
          ...
          <tr>
            <td height="25" valign="top" class="grey_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><h1>Manage Destinations Top Images </h1></td>
              </tr>
            </table></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <tr>
				...
				<?php  
					if($Data['image_name'])
					{
					echo '<a href="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/destSideImages/'.$Data['image_name'].'" 	rel="facebox">
              ...
              <tr>
				...
				<input type="button" name="cancel"  value="Cancel" class="btn" onClick="location.href='DestSideImages.php?destId=<?=$destId?>'">	
				</td>
              </tr>
            </table></td>
          </tr>
		...
		$webPage = $sql->SqlRecords("mos_destSideImages",$cond,$orderby,$offset=0,$records_per_page);
		...
		$Data = $webPage['Data'];		
          ...
          <tr>
			...
			<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center">
			  <tr>
				<td>
				<input type="button" class="btn" name="Add" value="Add New" onClick="document.location='?mode=add&destId=<?=$destId?>' ">
				</td>
				<td align="right">
			...
			<table width="98%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#66CCFF" bordercolor="#CCCCCC">
			  ...
			  <?php 
					if($count>0){
						for($i=0; $i<$count; $i++)
						{
							if($Data[$i]['image_name']!="" && file_exists(_UPLOAD_FILE_PATH."/dest_img/DestSideImages/".$Data[$i]['image_name']))
							{
							$img='<img src="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/DestSideImages/'.$Data[$i]['image_name'].'" border="0" width="100" height="100" />';
							}
							else
							{
							$img='<img src="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/DestSideImages/no_image_icon.jpg" border="0" width="100" height="100" />';
							}
					?>
			  <tr bgcolor="#FFFFFF">
			    <td align="center"><?=$img?></td>
			    <td align="center">
				<a href="DestSideImages.php?mode=edit&id=<?=$Data[$i]['id']?>&destId=<?=$destId?>"><img src="images/b_edit.png" border="0"></a>&nbsp;
				<a href="javascript: void(0)" onclick="SingleDelete('DestSideImages.php?delid=<?=$Data[$i]['id']?>&destId=<?=$destId?>');">				
