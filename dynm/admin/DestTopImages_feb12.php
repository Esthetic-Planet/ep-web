...

{
			...
			$arrDest_del = $sql->SqlSingleRecord('mos_destTopImages',$ConArr_del);
...

		{
			...
			$sql->SqlDelete('mos_destTopImages',$cond);	

			unlink(_UPLOAD_FILE_PATH."/dest_img/DestTopImages/".$Data_del['image_name']);

		echo "<body>";

		echo '<form name="frmSu" id="frmSu" method="post" action="'._ADMIN_WWWROOT.'DestTopImages.php">';
...
{		
		...
		$intResult = $sql->SqlInsert('mos_destTopImages',$ConArr);
...

			{
				...
				// in its *temporary* location in the server (often, it is /tmp)
					...
					//$handle->image_ratio_y        = true;
						...
						$handle->Process(_UPLOAD_FILE_PATH."/dest_img/DestTopImages/");
						...
						if ($handle->processed) {
							...
							$intResult = $sql->SqlUpdate('mos_destTopImages',$ConArr,$CondArr);
					...
					//$handle->image_ratio_y        = true;
						...
						if ($handle->processed) {
							...
							$intResult = $sql->SqlUpdate('mos_destTopImages',$ConArr,$CondArr);
					 		...
					 		@unlink(_UPLOAD_FILE_PATH."/dest_img/DestTopImages/".$_REQUEST['old_image_name']);
...
if(isset($_REQUEST['id']) && ($_REQUEST['mode']=='edit'))

	{
		...
		$arrBrands = $sql->SqlSingleRecord('mos_destTopImages',$ConArr);
...
<form name="frmAddDest" action="" method="post" onsubmit="return SubmitForm(this);" enctype="multipart/form-data">
          ...
          <tr>
            ...
            <tr>
				...
				<?php  
					...
					echo '<a href="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/DestTopImages/'.$Data['image_name'].'" 	rel="facebox">
              ...
              <tr>
				...
				<input type="button" name="cancel"  value="Cancel" class="btn" onClick="location.href='DestTopImages.php?destId=<?=$destId?>'">	
		...
		$webPage = $sql->SqlRecords("mos_destTopImages",$cond,$orderby,$offset=0,$records_per_page);
          ...
          <tr>
			...
			<table width="98%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#66CCFF" bordercolor="#CCCCCC">
			  ...
			  <?php 
					...
					if($count>0){
...

						{
							...
							if($Data[$i]['image_name']!="" && file_exists(_UPLOAD_FILE_PATH."/dest_img/DestTopImages/".$Data[$i]['image_name']))

							{

							$img='<img src="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/DestTopImages/'.$Data[$i]['image_name'].'" border="0" width="100" height="100" />';
							...
							$img='<img src="'._WWW_UPLOAD_IMAGE_PATH.'/dest_img/DestTopImages/no_image_icon.jpg" border="0" width="100" height="100" />';
			  ...
			  <tr bgcolor="#FFFFFF">
				...
				<a href="DestTopImages.php?mode=edit&id=<?=$Data[$i]['id']?>&destId=<?=$destId?>"><img src="images/b_edit.png" border="0"></a>&nbsp;

				<a href="javascript: void(0)" onclick="SingleDelete('DestTopImages.php?delid=<?=$Data[$i]['id']?>&destId=<?=$destId?>');">				
