					...
					{ 
                      ...
                      <tr <?=($i%2==0)? 'class="grey_bg"' : ""?>>
						...
						///get alldata from email except from form submitting in (surgery, ....) etc... 
						  echo $query_ulist_val_com = "SELECT distinct p.mail_sender,p.mail_reciever, u.* FROM esthp_mails as p, esthp_tblCustomers as u where  p.mail_reciever='$clid' and p.mail_sender Not IN(".$getuserIdimp.")  and p.mail_sender=u.cust_id";
						  }
						  else
						  {
						   echo $query_ulist_val_com = "SELECT distinct p.mail_sender,p.mail_reciever, u.* FROM esthp_mails as p, esthp_tblCustomers as u where  p.mail_reciever='$clid'  and p.mail_sender=u.cust_id";
						  } 
						$res_user_val_com = $sql->SqlExecuteQuery($query_ulist_val_com);
						$Data_plastic_com = $res_user_val_com["Data"];
						echo $count_plastic_com = $res_user_val_com["count"];
