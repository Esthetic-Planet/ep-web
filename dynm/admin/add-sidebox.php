...
{	
	...
	$record_id = $sql->SqlInsert('esthp_tblSideAds',$content_arr);
	...
	if($record_id)
	{
		if($_FILES['ad_image']['name']!="")
		{
				...
				if(move_uploaded_file($_FILES['ad_image']['tmp_name'], $upload_file))
				{
					...
					$sql->SqlUpdate('esthp_tblSideAds',$content_arr,$condition);
...
if(!empty($_REQUEST['ad_id']))
{
	...
	$ad_arr = $sql->SqlSingleRecord('esthp_tblSideAds',$cond);
	$count = $ad_arr['count'];
	$Data = $ad_arr['Data'];	
...
if(isset($_REQUEST['act']) && $_REQUEST['act']=='delimg')
{
	...
	header("location:add-sidebox.php?ad_id=".$ad_id);
}
...
<script language="javascript">
	...
	function delete_adImage()
	{
		...
		{
			window.location.href="add-sidebox.php?act=delimg&ad_id=<?=$ad_id?>"; 
		}
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td valign="top"><?php include_once('include/header.php');?><div id="breadcrumbs"> <a href="home.php">Home</a> &raquo; <a href="sidebox-ads.php">Side Ads</a> &raquo; Add Sidebox Ad</a></td></tr>
  <tr>
		   ...
		   <tr>
			...
			<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr>
				...
				if(is_file(_UPLOAD_FILE_PATH."ads/sidebox/".$ad_image))
				{

						if($ad_file_ext=='swf')
						{
						?><br/><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="300" height="250" title="test">
						  <param name="movie" value="<?=_UPLOAD_FILE_URL.'ads/sidebox/'.$Data['ad_image']?>" />
						  <param name="quality" value="high" />
						  <embed src="<?=_UPLOAD_FILE_URL.'ads/sidebox/'.$Data['ad_image']?>" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="300" height="250"></embed>
						</object><br/>&nbsp;
						<?php
						}
						else
						{
						?><a href="<?=_UPLOAD_FILE_URL?>ads/sidebox/<?=$ad_image?>" rel="facebox"><img border='0' src='<?=_ADMIN_IMAGE_PATH?>image_icon.gif' alt='Click to View'></a>
						<?php
						}
				?>&nbsp;<a href="javascript:void(0);" title="Click to Delete" onclick="javascript:delete_adImage()"><img border='0' src='<?=_ADMIN_IMAGE_PATH?>b_drop.png' alt='Click to Delete'></a>
				<?
				}
				?>
			...
			?></td>
				...
				<input type="button" name="cancel"  value="Cancel" class="btn" onClick="location.href='sidebox-ads.php'">				
